...
import Lab_3.Exceptions.TitleNameException;
import Lab_3.Moves.*;
...
public class Main implements actionsDunno, actionsDogs, actionsMoonshorties, createSubjects {
    public static void main(String[] args) {
        ...
        BadActions badActions = new BadActions() {
            @Override
            public String spit() {
                return " плюнул с досады,";
            }

            @Override
            public String shake() {
                return " с силой потряс кулаком над своей покрасневшей от злости лысиной";
            }

            @Override
            public String go() {
                return " и пошёл домой";
            }
        };
        ...
        try {
            title.showTitle(titleExist);
        } catch (TitleException e) {
            try{
                e.getMessage();
            }
            catch (TitleNameException exception){
                System.out.println(exception.getMessage());
            }
        }

        System.out.println("Он " + badActions.spit()+ badActions.shake() + badActions.go());
        ...
        nobody.notchasing();
        System.out.println(((Dunno) dunno).info());
    ...
    public Dunno createDunno() {
        return new Dunno("Незнайка", "хорошее");
    }
